...
void PNGImageDestroy(long img)
{
  ...
  if(i > -1)
  {
    ...
    png_destroy_read_struct(&imgsbuf[i].png_ptr, &imgsbuf[i].info_ptr, &imgsbuf[i].end_info);
...
int CopyImage(HDC hdc, png_byte **rows, int width, int height, BackgroundFunction backfnc)
{
  ...
  for(int j = 0; j < height; j++)
  {
    ...
    for(int i = 0; i < width; i++)
    {
      ...
      alpha = row[4*i + 3];
      pbresrow[4*i + 2] = ((255 - alpha)*((bkcol >> 16) & 0xFF) + alpha*row[4*i])/255;
      pbresrow[4*i + 1] = ((255 - alpha)*((bkcol >> 8) & 0xFF) + alpha*row[4*i + 1])/255;
      pbresrow[4*i] = ((255 - alpha)*(bkcol & 0xFF) + alpha*row[4*i + 2])/255;
      pbresrow[4*i + 3] = 0;
    }
    pbresrow += resrowsize;
  }

  SetDIBitsToDevice(hdc, 0, 0, width, height, 0, 0, 0, height, pbres, &bif, DIB_RGB_COLORS);
  free(pbres);
  return 0;
}
  ...
  int dstwidth, int dstheight, BackgroundFunction backfnc)
{
  ...
  for(int j = 0; j < dstheight; j++)
  {
    for(int i = 0; i < dstwidth; i++)
    {
      if(backfnc) bkcol = backfnc(i, j);
      alpha = pbmaskrow[4*i];
      pbcolrow[4*i + 2] = ((255 - alpha)*((bkcol >> 16) & 0xFF) + alpha*pbcolrow[4*i + 2])/255;
      pbcolrow[4*i + 1] = ((255 - alpha)*((bkcol >> 8) & 0xFF) + alpha*pbcolrow[4*i + 1])/255;
      pbcolrow[4*i] = ((255 - alpha)*(bkcol & 0xFF) + alpha*pbcolrow[4*i])/255;
      pbcolrow[4*i + 3] = 0;
    }
    pbcolrow += outrowsize;
  ...
  int desth, BackgroundFunction backfnc)
{
  ...
  if(i > -1)
  {
    ...
    if(imgsbuf[i].color_type == PNG_COLOR_TYPE_GRAY && imgsbuf[i].bit_depth < 8)
      png_set_expand_gray_1_2_4_to_8(imgsbuf[i].png_ptr);
    if(imgsbuf[i].color_type == PNG_COLOR_TYPE_RGB || imgsbuf[i].color_type == PNG_COLOR_TYPE_GRAY)
      png_set_add_alpha(imgsbuf[i].png_ptr, 255, PNG_FILLER_AFTER);
    if(png_get_valid(imgsbuf[i].png_ptr, imgsbuf[i].info_ptr, PNG_INFO_tRNS))
...
BOOL PNGImageGetBitmaps(long img, HDC ADC, HBITMAP hBmpColor, HBITMAP hBmpMask)
{
  int i = FindImage(img);
  if(i > -1)
  {
