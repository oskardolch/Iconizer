...
LONG_PTR CALLBACK ImageProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  {
  case WM_SETCURSOR:
    return paid->ImageWMSetCursor(hWnd, (HWND)wParam, LOWORD(lParam), HIWORD(lParam));
  case WM_LBUTTONDOWN:
    return paid->ImageWMLButtonDown(hWnd, wParam, LOWORD(lParam), HIWORD(lParam));
  case WM_LBUTTONUP:
    return paid->ImageWMLButtonUp(hWnd, wParam, LOWORD(lParam), HIWORD(lParam));
  case WM_MOUSEMOVE:
    return paid->ImageWMMouseMove(hWnd, wParam, LOWORD(lParam), HIWORD(lParam));
  default:
...
BOOL CAddImageDlg::ShowModal(HWND hWnd, LPTSTR sFileName, PADDIMGDATA pAddImgData)
{
  ...
  return DialogBoxParam(m_hInstance, _T("ADDIMGDLG"), hWnd, AddImgDlgProc, (LPARAM)this) == IDOK;
}
...
INT_PTR CAddImageDlg::WMInitDialog(HWND hWnd, HWND hwndFocus, LPARAM lInitParam)
{
  ...
  SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHBDITHER, BM_SETCHECK, BST_CHECKED, 0);
...
void CAddImageDlg::LoadImage(HWND hWnd, HWND hwndCtl)
{
  ...
  if(fp)
  {
    ...
    {
      // bitmap
      m_Bmp = RenderBitmap(hwndCtl, m_bData, m_AddImgData->dwTranspColor, &iWidth, &iHeight);
    ...
    SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_EDTWIDTH, WM_SETTEXT, 0, (LPARAM)buf);
    _stprintf(buf, _T("%d"), iHeight);
    SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_EDTHEIGHT, WM_SETTEXT, 0, (LPARAM)buf);
  }
  return;
}

LONG_PTR CAddImageDlg::ImageWMSetCursor(HWND hWnd, HWND hwnd, WORD nHittest, WORD wMouseMsg)
{
  if(m_bSelectCol) SetCursor(LoadCursor(0, IDC_CROSS));
  return 1;
}

LONG_PTR CAddImageDlg::ImageWMLButtonDown(HWND hWnd, WPARAM fwKeys, WORD xPos, WORD yPos)
{
...
LONG_PTR CAddImageDlg::ImageWMLButtonUp(HWND hWnd, WPARAM fwKeys, WORD xPos, WORD yPos)
{
...
LONG_PTR CAddImageDlg::ImageWMMouseMove(HWND hWnd, WPARAM fwKeys, WORD xPos, WORD yPos)
{
  ...
  if(m_ptImageStartClick.x > -1)
  {
    Rectangle(m_ImageDC, m_ptImageStartClick.x, m_ptImageStartClick.y,
      m_ptImageMove.x, m_ptImageMove.y);
    m_bHasMoved |= abs(m_ptImageMove.x - dx) > 2 || abs(m_ptImageMove.y - dy) > 2;
    m_ptImageMove.x = dx;
    m_ptImageMove.y = dy;
    Rectangle(m_ImageDC, m_ptImageStartClick.x, m_ptImageStartClick.y,
      m_ptImageMove.x, m_ptImageMove.y);
...
void CAddImageDlg::SetTransparentColor(HWND hWnd, HWND hwndImg)
{
  ...
  {
    if(m_Bmp) DeleteObject(m_Bmp);
    m_Bmp = RenderBitmap(hwndImg, m_bData, m_AddImgData->dwTranspColor, NULL, NULL);
  }
  return;
}
...
INT_PTR CAddImageDlg::OKBtnClick(HWND hWnd)
{
  ...
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHB32, BM_GETCHECK, 0, 0) == BST_CHECKED)
    m_AddImgData->bSize |= AID_SIZE_32;
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHB24, BM_GETCHECK, 0, 0) == BST_CHECKED)
    m_AddImgData->bSize |= AID_SIZE_24;
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHB16, BM_GETCHECK, 0, 0) == BST_CHECKED)
    m_AddImgData->bSize |= AID_SIZE_16;
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHBCUSTOM, BM_GETCHECK, 0, 0) == BST_CHECKED)
  {
  ...
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHBPNG, BM_GETCHECK, 0, 0) == BST_CHECKED)
    m_AddImgData->bColorDepth |= AID_DEPTH_PNG;
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHBRGBA, BM_GETCHECK, 0, 0) == BST_CHECKED)
    m_AddImgData->bColorDepth |= AID_DEPTH_RGBA;
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHB8BIT, BM_GETCHECK, 0, 0) == BST_CHECKED)
    m_AddImgData->bColorDepth |= AID_DEPTH_8;
  if(SendDlgItemMessage(hWnd, ADDIMGDLG_CTLR_ACHB4BIT, BM_GETCHECK, 0, 0) == BST_CHECKED)
    m_AddImgData->bColorDepth |= AID_DEPTH_4;
