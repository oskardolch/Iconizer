...
INT_PTR CALLBACK EditImgDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
void CEditImageDlg::SetupPalette(HWND hWnd)
{
  ...
  SetWindowPos(GetDlgItem(hWnd, EDTIMGDLG_CTLR_GBPALETTE), NULL, R.left, R.top, R.right, R.bottom, SWP_NOZORDER);
  SetWindowPos(GetDlgItem(hWnd, IDOK), NULL, 4, R.top + R.bottom + 4, 0, 0, SWP_NOSIZE);
  if(columns > 8)
    SetWindowPos(GetDlgItem(hWnd, IDCANCEL), NULL, 84, R.top + R.bottom + 4, 0, 0, SWP_NOSIZE);
  else
    SetWindowPos(GetDlgItem(hWnd, IDCANCEL), NULL, 4, R.top + R.bottom + 32, 0, 0, SWP_NOSIZE);
  ...
  SetWindowPos(GetDlgItem(hWnd, EDTIMGDLG_CTLR_GBIMAGE), NULL, R.right, R.top, 0, 0, SWP_NOSIZE);
  SetupImage(hWnd);
  return;
}
...
void CEditImageDlg::DrawImage(HWND hWnd, HDC hdc)
{
  ...
  StretchDIBits(hdc, m_ptImgOrig.x, m_ptImgOrig.y, m_iZoom*srcw, m_iZoom*srch,
    0, 0, srcw, srch, m_pImage->bmiColors, m_pImage, DIB_RGB_COLORS, SRCCOPY);
  return;
}
...
INT_PTR CEditImageDlg::ZoomOut(HWND hWnd, HWND hwndCtl)
{
  m_iZoom -= 2;
  HWND tb = GetDlgItem(hWnd, EDTIMGDLG_CTLR_TOOLBAR);
  SendMessage(tb, TB_SETSTATE, EDTIMGDLG_CTLR_ZOOMINBNT, MAKELONG(TBSTATE_ENABLED, 0));
  if(m_iZoom < 2)
  {
...
int CEditImageDlg::IsImagePoint(HWND hWnd, LPPOINT ppt)
{
  ...
  row /= m_iZoom;
  col /= m_iZoom;
  if(row >= m_pImage->bmiHeader.biWidth || col >= m_pImage->bmiHeader.biHeight)
    return -1;
