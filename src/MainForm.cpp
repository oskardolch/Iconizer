...
CMainForm::CMainForm(HINSTANCE hInstance, DLGPROC lpDialogFunc, HWND *phWnd, HACCEL *phAccTable)
{
  ...
  *phWnd = CreateDialogParam(m_hInstance, _T("MAINFORM"), 0, lpDialogFunc, (LPARAM)this);
...
INT_PTR CMainForm::WMUnknownMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if(fp)
  {
    ...
    if((icd.idReserved == 0) && (iType > 0))
    {
      ...
      for(int i = 0; i < icd.idCount; i++)
      {
        ...
        lpid = (LPICONDATA)malloc(sizeof(HBITMAP) + sizeof(icdi) + icdi.dwBytesInRes);
        lpid->hPreview = NULL;
...
void CMainForm::ClearListBox(HWND hWnd)
{
  ...
  SendDlgItemMessage(hWnd, CTRL_IMAGE, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)NULL);
  return;
}
...
INT_PTR CMainForm::ListBoxDragMsg(HWND hWnd, int idCtl, LPDRAGLISTINFO pDragInfo)
{
  ...
  {
  case DL_BEGINDRAG:
    return ListBoxBeginDrag(hWnd, idCtl, pDragInfo->hWnd, pDragInfo->ptCursor);
  case DL_CANCELDRAG:
    return ListBoxCancelDrag(hWnd, idCtl, pDragInfo->hWnd, pDragInfo->ptCursor);
  case DL_DRAGGING:
    return ListBoxDragging(hWnd, idCtl, pDragInfo->hWnd, pDragInfo->ptCursor);
  case DL_DROPPED:
    return ListBoxDropped(hWnd, idCtl, pDragInfo->hWnd, pDragInfo->ptCursor);
  default:
    return 0;
  }
}

INT_PTR CMainForm::ListBoxBeginDrag(HWND hWnd, int idCtl, HWND hwndCtl, POINT ptCursor)
{
...
INT_PTR CMainForm::ListBoxCancelDrag(HWND hWnd, int idCtl, HWND hwndCtl, POINT ptCursor)
{
  m_iDragItem = -1;
...
INT_PTR CMainForm::ListBoxDragging(HWND hWnd, int idCtl, HWND hwndCtl, POINT ptCursor)
{
...
INT_PTR CMainForm::ListBoxDropped(HWND hWnd, int idCtl, HWND hwndCtl, POINT ptCursor)
{
...
#define IDS_PNG_FILTER 6
    ...
    ofn.lpstrInitialDir = m_sLastOpenPath;
    ofn.lpstrTitle = NULL;
    ofn.Flags = OFN_OVERWRITEPROMPT | OFN_NONETWORKBUTTON | OFN_EXPLORER | OFN_ENABLESIZING;
    ofn.nFileOffset = 0;
    ofn.nFileExtension = 0;
...
LPICONDATA CMainForm::IconDataFromAddImgData(PADDIMGDATA pAid, int iBitDepth, int iWidth, int iHeight)
{
...
void CMainForm::InsertIconImages(HWND hWnd, PADDIMGDATA pAid)
{
    ...
    if(pAid->bColorDepth & (1 << i))
    {
      ...
      for(int j = 0; j < 7; j++)
      {
        if(pAid->bSize & (1 << j))
        {
          ...
          if(lpid)
          {
            _stprintf(buf, _T("%dx%d - %d bpp"), isize, isize, idepth);
      ...
      {
        lpid = IconDataFromAddImgData(pAid, idepth, pAid->iCustWidth, pAid->iCustHeight);
        if(lpid)
        {
          _stprintf(buf, _T("%dx%d - %d bpp"), pAid->iCustWidth, pAid->iCustHeight, idepth);
          idx = SendMessage(lb, LB_ADDSTRING, 0, (LPARAM)buf);
          SendMessage(lb, LB_SETITEMDATA, (WPARAM)idx, (LPARAM)lpid);
        }
      }
  ...
  if((pAid->bColorDepth & AID_DEPTH_PNG) && (pAid->bImgType == 1))
  {
    ...
    if(lpid)
    {
      ...
      _stprintf(buf, _T("%dx%d - %d bpp"), width, height, lpid->icdi.wBitCount);
      idx = SendMessage(lb, LB_ADDSTRING, 0, (LPARAM)buf);
      SendMessage(lb, LB_SETITEMDATA, (WPARAM)idx, (LPARAM)lpid);
    }
  }
