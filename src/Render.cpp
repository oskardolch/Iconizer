...
HBITMAP RenderIcon(HWND hWndStatic, LPICONIMAGE pIcoImg, int width, int height)
{
    ...
    HBITMAP hbmpRes = CreateDIBitmap(hdc, &bif.bmiHeader, CBM_INIT,
        pbres, &bif, DIB_RGB_COLORS);
...
HBITMAP RenderPNG(HWND hWndStatic, LPICONIMAGE pIcoImg, int iSize, int *piWidth,
    int *piHeight)
{
/*wchar_t buf[64];
...
//MessageBox(0, L"Core", L"Debug", MB_OK);
    ...
    else if(pbci->bcSize == sizeof(BITMAPINFOHEADER))
    {
        ...
        if(srcw == dstw && srch == dsth)
            hbmpRes = CreateDIBitmap(hdc, &pbi->bmiHeader, CBM_INIT,
...
LPICONDATA PNGtoIconCopy(char *pData, int iDataSize)
{
    ...
    if(PNGImageIsPNG(lpng))
    {
        ...
        icdRes = (LPICONDATA)malloc(sizeof(HBITMAP) + sizeof(ICONDIRENTRY) +
            iDataSize);
...
int GetPalIndex(RGBQUAD *pPal, int iPalSize, BYTE r, BYTE g, BYTE b)
{
    ...
    {
        curdist = DistFunc(r - pPal[i].rgbRed, g - pPal[i].rgbGreen,
            b - pPal[i].rgbBlue);
        if(curdist < mindist)
        {
            mindist = curdist;
    ...
    for(int j = 0; j < iHeight; j++)
    {
        ...
        for(int i = 0; i < iWidth; i++)
        {
            ...
            else if(bDither)
            {
                ...
                {
                    imgbufrow[4*(i + 1)] =
                        AddByte(imgbufrow[4*(i + 1)], Round(7*db));
                    imgbufrow[4*(i + 1) + 1] =
                        AddByte(imgbufrow[4*(i + 1) + 1], Round(7*dg));
                    imgbufrow[4*(i + 1) + 2] =
                        AddByte(imgbufrow[4*(i + 1) + 2], Round(7*dr));
                }
                if(imgbufnextrow)
                {
                    if(i > 0)
                    {
                        imgbufnextrow[4*(i - 1)] =
                            AddByte(imgbufnextrow[4*(i - 1)], Round(3*db));
                        imgbufnextrow[4*(i - 1) + 1] =
                            AddByte(imgbufnextrow[4*(i - 1) + 1], Round(3*dg));
                        imgbufnextrow[4*(i - 1) + 2] =
                            AddByte(imgbufnextrow[4*(i - 1) + 2], Round(3*dr));
                    }
                    imgbufnextrow[4*i] =
                        AddByte(imgbufnextrow[4*i], Round(5*db));
                    imgbufnextrow[4*i + 1] =
                        AddByte(imgbufnextrow[4*i + 1], Round(5*dg));
                    imgbufnextrow[4*i + 2] =
                        AddByte(imgbufnextrow[4*i + 2], Round(5*dr));
                    if(i < iWidth - 1)
                    {
                        imgbufnextrow[4*(i + 1)] =
                            AddByte(imgbufnextrow[4*(i + 1)], Round(db));
                        imgbufnextrow[4*(i + 1) + 1] =
                            AddByte(imgbufnextrow[4*(i + 1) + 1], Round(dg));
                        imgbufnextrow[4*(i + 1) + 2] =
                            AddByte(imgbufnextrow[4*(i + 1) + 2], Round(dr));
                    }
                }
            }
            // end of FSD
    ...
    int iWidth, int iHeight, BOOL bDither, BOOL bOptimize)
{
    ...
    if(PNGImageIsPNG(lpng))
    {
        ...
        StretchBlt(dstdc, 0, 0, iWidth, iHeight, srcdc, 0, 0, srcw, srch,
            SRCCOPY);
        ...
        GetAlphaImage(dstdc, bmpDstColor, bmpDstMask, iWidth, iHeight,
            pAlphaImg);
    ...
    else if(pbci->bcSize == sizeof(BITMAPINFOHEADER))
    {
        ...
        else
        {
            ...
            for(int j = 0; j < srch; j++)
            {
                ...
                {
                    col = maskBitsRow[4*i + 2] | (maskBitsRow[4*i + 1] << 8) |
                        (maskBitsRow[4*i] << 16);
